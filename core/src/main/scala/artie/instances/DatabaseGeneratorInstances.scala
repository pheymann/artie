...
trait DatabaseGeneratorInstances {
  ...
  implicit def dbSingleGen[A](implicit colReader: Col[A]) = new DatabaseGenerator[A] {
    def apply(query: String, config: DatabaseConfig): Seq[A] =
      runQuery(query, config)(rows => colReader(rows)(1))
  }

  implicit def dbTuple2Gen[A, B](implicit colA: Col[A], colB: Col[B]) = new DatabaseGenerator[(A, B)] {
    def apply(query: String, config: DatabaseConfig): Seq[(A, B)] =
      runQuery(query, config)(rows => (colA(rows)(1), colB(rows)(2)))
  }

  implicit def dbTuple3Gen[A, B, C](implicit colA: Col[A], colB: Col[B], colC: Col[C]) = new DatabaseGenerator[(A, B, C)] {
    def apply(query: String, config: DatabaseConfig): Seq[(A, B, C)] =
      runQuery(query, config)(rows => (colA(rows)(1), colB(rows)(2), colC(rows)(3)))
  }
  ...
  implicit def dbTuple4Gen[A, B, C, D](implicit colA: Col[A],
    ...
    def apply(query: String, config: DatabaseConfig): Seq[(A, B, C, D)] =
      runQuery(query, config)(rows => (colA(rows)(1), colB(rows)(2), colC(rows)(3), colD(rows)(4)))
  }
  ...
  implicit def dbTuple5Gen[A, B, C, D, E](implicit colA: Col[A],
    ...
    def apply(query: String, config: DatabaseConfig): Seq[(A, B, C, D, E)] =
      runQuery(query, config)(rows => (colA(rows)(1), colB(rows)(2), colC(rows)(3), colD(rows)(4), colE(rows)(5)))
  }
}
